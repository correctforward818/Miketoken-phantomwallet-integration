...
export const initialState: UserState = {
  ...
  watchlistTokens: [],
  watchlistPools: [],
  hideTimestampPhishingWarningBanner: null as any,
  walletType: "",
  solanaAddress: "",
};
...
export default createReducer(initialState, (builder) =>
  builder
      ...
      (state, { payload: { chainId, tokenAAddress, tokenBAddress } }) => {
        if (state.pairs[chainId]) {
          ...
          state.pairs[chainId] = omitBy(
            state.pairs[chainId],
            (_value, key) => key === tokenAToB || key === tokenBToA
          );
        }
      }
    )
    ...
    .addCase(updateWalletType, (state, { payload: { type } }) => {
      state.walletType = type;
    })
    .addCase(updateSolanaAddress, (state, { payload: { address } }) => {
